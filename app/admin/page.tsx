...
import { BriefcaseBusiness, FileEdit, Gauge, ImageIcon, LogOut, Plus, Settings2, ShieldCheck, Users } from 'lucide-react'
      ...
      <div className="mx-auto max-w-7xl">
        <header className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {process.env.PHASE4_PORTAL_ADMIN_ENABLED === 'true' && <Link href="/admin/portal" className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-5 py-3 text-sm font-bold"><Users className="h-4 w-4" />Portals</Link>}
            {process.env.PHASE5_ADMIN_ENABLED === 'true' && <Link href="/admin/phase5" className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-5 py-3 text-sm font-bold"><Gauge className="h-4 w-4" />Operating centre</Link>}
