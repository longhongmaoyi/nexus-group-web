...
export function StrategicServicePage({
  locale,
  page,
}: {
  locale: Locale
      ...
      <main className="min-h-screen bg-[#f4f1e9] text-[#11191b]">
        <section className="relative overflow-hidden bg-[#082328] pb-20 pt-40 text-white">
          ...
          <div className="relative mx-auto max-w-[1320px] px-5 sm:px-8 lg:px-12">
            <p className="eyebrow text-[#b8d683]">{text(page.eyebrow)}</p>
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link href={`/${locale}${page.primaryHref}`} className="premium-button-light">
                {text(page.primaryLabel)}
                <ArrowUpRight className="h-4 w-4" />
              </Link>
              <Link href={`/${locale}${page.secondaryHref}`} className="premium-button-ghost">
        ...
        <section className="mx-auto max-w-[1320px] px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
          ...
          <div className="mt-7 grid gap-px bg-black/10 md:grid-cols-2">
            {page.audience.map((item) => (
              <div key={text(item)} className="flex gap-3 bg-white p-5 sm:p-6">
                <Check className="mt-1 h-4 w-4 shrink-0 text-[#176b96]" />
                <p className="leading-7 text-slate-700">{text(item)}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="border-y border-black/10 bg-white px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
          <div className="mx-auto max-w-[1320px]">
            <div className="grid gap-6 md:grid-cols-2">
              {page.sections.map((section, index) => (
                <article
                  ...
                  className={index === page.sections.length - 1 && page.sections.length % 2 === 1
                    ? 'border border-slate-200 bg-[#f8faf9] p-6 sm:p-8 md:col-span-2'
                    : 'border border-slate-200 bg-[#f8faf9] p-6 sm:p-8'}
                >
                  <span className="text-xs font-bold tracking-[0.18em] text-[#176b96]">
                  ...
                  <div className="mt-6 grid gap-3">
                    {section.points.map((point) => (
                      <p key={text(point)} className="flex items-start gap-3 text-sm leading-6 text-slate-700">
                        <Check className="mt-1 h-4 w-4 shrink-0 text-[#176b96]" />
                        {text(point)}
                      </p>
                    ))}
            ...
            {page.note ? (
              <div className="mt-8 border border-amber-200 bg-amber-50 p-5 text-sm leading-7 text-amber-950">
                {text(page.note)}
              </div>
            ) : null}
          </div>
        ...
        <section className="bg-[#176b96] px-5 py-16 text-white sm:px-8 lg:px-12 lg:py-20">
          <div className="mx-auto flex max-w-[1320px] flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em] text-white/65">
                NEXUS / NEXT STEP
              </p>
              <h2 className="mt-4 max-w-3xl text-4xl font-semibold tracking-[-0.045em]">
                {text(page.primaryLabel)}
              </h2>
            </div>
