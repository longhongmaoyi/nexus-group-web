    ...
    <main className="min-h-screen bg-[#f4f1e9] text-[#11191b]">
      ...
      <section className="mx-auto max-w-[1320px] px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
        ...
        <div className="mt-7 grid gap-px bg-black/10 md:grid-cols-2">
          {page.audience.map((item) => <div key={text(item)} className="flex gap-3 bg-white p-5 sm:p-6"><Check className="mt-1 h-4 w-4 shrink-0 text-[#176b96]" /><p className="leading-7 text-slate-700">{text(item)}</p></div>)}
        </div>
      </section>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {page.sections.map((section, index) => (
              <article key={text(section.title)} className={index === page.sections.length - 1 && page.sections.length % 2 === 1 ? 'border border-slate-200 bg-[#f8faf9] p-6 sm:p-8 md:col-span-2' : 'border border-slate-200 bg-[#f8faf9] p-6 sm:p-8'}>
                <span className="text-xs font-bold tracking-[0.18em] text-[#176b96]">{String(index + 1).padStart(2, '0')}</span>
                ...
                <div className="mt-6 grid gap-3">{section.points.map((point) => <p key={text(point)} className="flex items-start gap-3 text-sm leading-6 text-slate-700"><Check className="mt-1 h-4 w-4 shrink-0 text-[#176b96]" />{text(point)}</p>)}</div>
              </article>
            ))}
          </div>
