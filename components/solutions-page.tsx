    ...
    <main className="min-h-screen bg-cream text-ink">
      <section className="relative overflow-hidden bg-ink pb-20 pt-40 text-white">
        ...
        <div className="relative mx-auto max-w-[1320px] px-5 sm:px-8 lg:px-12">
          <p className="eyebrow text-brand-frost">{text(solutionsCopy.eyebrow)}</p>
      ...
      <nav className="sticky top-0 z-40 border-b border-black/10 bg-cream/95 backdrop-blur" aria-label={text(solutionsCopy.title)}>
        <div className="mx-auto flex max-w-[1320px] gap-2 overflow-x-auto px-5 py-3 sm:px-8 lg:px-12">
          {solutionSections.map((section) => <a key={section.id} href={`#${section.id}`} className="shrink-0 border border-black/15 bg-white px-4 py-2 text-xs font-bold uppercase tracking-[0.08em] transition hover:border-brand hover:text-brand">{text(section.title)}</a>)}
        </div>
      </nav>
      <section id="industries" className="mx-auto max-w-[1320px] scroll-mt-24 px-5 py-16 sm:px-8 lg:px-12 lg:py-24">
        ...
        <div className="mt-8 grid gap-px bg-black/10 sm:grid-cols-2 lg:grid-cols-3">
          {solutionSections.map((section) => <a key={section.id} href={`#${section.id}`} className="bg-white p-5 transition hover:bg-black/5"><p className="font-semibold">{text(section.title)}</p><p className="mt-2 text-sm leading-6 text-slate-600">{text(section.summary)}</p></a>)}
        </div>
      </section>
      <div className="border-t border-black/10">
        {solutionSections.map((section, index) => (
          <section key={section.id} id={section.id} className={`scroll-mt-24 px-5 py-16 sm:px-8 lg:px-12 lg:py-24 ${index % 2 ? 'bg-white' : 'bg-cream'}`}>
            <div className="mx-auto max-w-[1320px]">
              <div className="grid gap-8 lg:grid-cols-[0.34fr_0.66fr]">
                <div><p className="text-xs font-bold tracking-[0.18em] text-brand">{String(index + 1).padStart(2, '0')}</p><h2 className="mt-4 text-4xl font-semibold tracking-[-0.045em] sm:text-5xl">{text(section.title)}</h2></div>
              ...
              <div className="mt-10 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                {[
                  ...
                  <article key={text(label as Record<Locale,string>)} className="border border-slate-200 bg-[#f8faf9] p-5 sm:p-6">
                    <h3 className="text-sm font-bold uppercase tracking-[0.12em] text-brand">{text(label as Record<Locale,string>)}</h3>
                    <div className="mt-4 grid gap-3">{(items as Array<Record<Locale,string>>).map((entry) => <p key={text(entry)} className="flex items-start gap-3 text-sm leading-6 text-slate-700"><Check className="mt-1 h-4 w-4 shrink-0 text-brand" />{text(entry)}</p>)}</div>
                  </article>
                ))}
              </div>
